...
class Actor(tf.keras.Model):
    def __init__(self, state_feature_dim, action_dim, max_action, layer_units=(400, 300), name="Actor"):
        ...
        for cur_layer in layer_units:
            base_layers.append(layers.Dense(cur_layer))

        self.base_layers = base_layers
        self.out_layer = layers.Dense(action_dim)
...
class Critic(tf.keras.Model):
    def __init__(self, state_action_feature_dim, layer_units=(400, 300), name="Critic"):
        ...
        self.out1 = layers.Dense(1, name="out1")
        self.out2 = layers.Dense(1, name="out2")
...
class TD3(tracking.AutoTrackable):
    def __init__(self, state_dim, action_dim, max_action, feature_extractor, layer_units=(400, 300), action_noise=0.1):
    ...
    def _critic_update(self, states, actions, next_states, rewards, done, discount, policy_noise=0.2, noise_clip=0.5):
        with tf.device("/gpu:0"):
            ...
            action_noise = tf.random.normal(noise_shape, mean=0.0, stddev=policy_noise)
            ...
            with tf.GradientTape() as tape:
                ...
                # Compute critic loss
                critic_loss = tf.reduce_mean(losses.MSE(current_Q1, target_Q)) + tf.reduce_mean(
                    losses.MSE(current_Q2, target_Q)
                )
