...
from teflon.ofe.blocks import DensenetBlock, MLPBlock, ResnetBlock
...
class OFENet(tf.keras.Model):
    def __init__(
        ...
        state_layer_units, action_layer_units = calculate_layer_units(
            dim_state, dim_action, block, total_units, num_layers
        )
        self.act = activation
        ...
        if block == "resnet":
            ...
            for i in range(len(state_layer_units) // 2):
                cur_block = block_class(
                    units1=state_layer_units[i * 2],
                    units2=state_layer_units[i * 2 + 1],
                    activation=self.act,
            ...
            for i in range(len(action_layer_units) // 2):
                cur_block = block_class(
                    units1=action_layer_units[i * 2],
                    units2=action_layer_units[i * 2 + 1],
                    activation=self.act,
                    kernel_initializer=kernel_initializer,
                    batchnorm=batchnorm,
                    trainable=trainable,
        ...
        self.out_layer = layers.Dense(dim_output, trainable=trainable, name="feat_out")
        ...
        logger.debug(
            "state feature dim is {} (state-dim: {}, add: {})".format(self.dim_state_features, dim_state, add_dim)
        )
        add_dim = self.dim_state_action_features - (dim_action + self.dim_state_features)
        logger.debug(
            "state-action feature dim is {} (action-dim: {}, add: {})".format(
