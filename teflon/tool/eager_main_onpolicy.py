...
from teflon.tool.eager_main import feature_extractor, make_exp_name, make_output_dir
from teflon.util import replay
from teflon.util.misc import EmpiricalNormalization, set_gpu_device_growth

set_gpu_device_growth()


class Trainer:
    def __init__(self, policy, env, eval_env, checkpoint_manager, dim_state, dim_action, ofe_batchsize=256):
        self._policy = policy
        self._ofe_batchsize = ofe_batchsize
        self._policy = policy
        self._normalizer = EmpiricalNormalization(shape=(dim_state,), clip_threshold=5.0)
        self._env = env
    ...
    def pretrain_ofe(self, extractor, dim_state, dim_action, random_collect=10000):
        replay_buffer = replay.ReplayBuffer(state_dim=dim_state, action_dim=dim_action, capacity=random_collect)
        episode_timesteps = 0
...
def discount_cumsum(x, discount):
    ...
    :return output:
        [x0 + discount * x1 + discount^2 * x2,
         ...
         x2]
    """
    return lfilter(b=[1], a=[1, float(-discount)], x=x[::-1], axis=0)[::-1]
...
def main():
    ...
    trainer = Trainer(policy, env, eval_env, checkpoint_manager, dim_state, dim_action)
    if random_collect > 0:
