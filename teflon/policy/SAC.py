...
class SAC(tracking.AutoTrackable):
    def __init__(
        ...
        self.actor = GaussianActor(
            feature_extractor.dim_state_features, action_dim, max_action, layer_units=actor_units
        )
    ...
    def train_for_batch(self, states, actions, rewards, next_states, done, discount=0.99):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Equation (7, 8)
            ys = tf.stop_gradient(self.scale_reward * rewards + not_done * discount * vf_next_target_t)
    ...
    def train(self, replay_buffer, batch_size=256, discount=0.99):
        ...
        with tf.device("/gpu:0"):
            policy_loss, vf_loss, td_loss = self.train_for_batch(
                states, actions, rewards, next_states, dones, discount=discount
            )
